...
Espo.define('pim:views/attribute/record/panels/product-family-attributes', ['views/record/panels/relationship', 'views/record/panels/bottom', 'search-manager'],
                    ...
                    collection.once('sync', function () {
                        this.createView('list', viewName, {
                            ...
                            view.listenTo(view, 'after:render', () => {
                                ...
                                for (let key in rows) {
                                    ['isRequired', 'isMultiChannel'].forEach(fieldName => {
                                        let field = rows[key].getView(fieldName + 'Field');
                                        field.setMode('edit');
                                        field.reRender();

                                        view.stopListening(rows[key].model, `change:${fieldName}`);
                                        view.listenTo(rows[key].model, `change:${fieldName}`, model => {
                                            this.notify('Saving...');
                                            this.ajaxPutRequest('ProductFamily/action/updateAttribute', {
                                                [fieldName]: model.get(fieldName),
                                                attributeId: this.model.id,
                                                productFamilyId: model.id
                                            })
