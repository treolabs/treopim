...
**[Locale](https://treopim.com/store/multi-language-and-local-fields)** – a combination of a language (i.e. German, English) and a country/region (i.e. Germany, United Kingdom, the U.S.A.) preferred to be used in the interface. For instance, German German is marked as "de_DE", Austrian German is marked as "de_AT", UK English is marked as "en_GB" and so on. You can use one or more locales in the system. 
 ...
 > This functionality is enabled when the  **"Multilanguage and Locales"** module is installed to your system. Please, see the details in the [TreoPIM store](https://treopim.com/store/multi-language-and-local-fields).
...
**[Dashboard**](https://treopim.com/help/user-interface#dashboard) – a collection of data displayed in a graphical or table layout as widgets. Dashboards allow users to have all the important information grouped by a certain type or nature in one single place. Every user can have several dashboards.
